...
export default function BudgetPlanner() {
  ...
  const [selectedMonth, setSelectedMonth] = useState(
    new Date().toISOString().slice(0, 7) // e.g., "2025-04"
  );
  ...
  const onBudgetSubmit = async (values) => {
    try {
      ...
      toast("Budget Saved.",{
        description: `Set ${values.category} budget for ${values.month} to $${Number(values.amount).toFixed(2)}.`,
        duration: 5000,
      })
  ...
  const handleDeleteBudget = async (id) => {
    try {
      ...
      toast("Budget Deleted.",{
        description: "Budget delete successfull.",
        duration: 5000,
      })
    } catch (error) {
    ...
    <div>
      ...
      <main className="flex min-h-screen flex-col items-center p-24">
        ...
        <div className="w-full max-w-4xl space-y-12">
          ...
          {/* Current Budgets */}
          <div>
            <h2 className="text-lg font-semibold mb-4">Current Budgets for {selectedMonth}</h2>
            {loading ? (
